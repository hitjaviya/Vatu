...
class MessageService {
    ...
    /**
     * Get the latest bucket number for a conversation (for bucket-based pagination)
     */
    static async getLatestBucketNumber(user1Id, user2Id = null, groupId = null) {
        return monitorPerformance('getLatestBucketNumber', async () => {
            const conversationId = groupId
                ? generateGroupConversationId(groupId)
                : generateConversationId(user1Id, user2Id);
            return ConversationBucket.getLatestBucketNumber(conversationId);
        });
    }

    /**
     * Get all messages from a specific bucket (for bucket-based pagination)
     */
    static async getMessagesByBucket(user1Id, user2Id = null, groupId = null, bucketNumber) {
        return monitorPerformance('getMessagesByBucket', async () => {
            const conversationId = groupId
                ? generateGroupConversationId(groupId)
                : generateConversationId(user1Id, user2Id);

            const result = await ConversationBucket.getMessagesByBucket(conversationId, bucketNumber);
            return {
                messages: result.messages.map(msg => formatMessage(msg)),
                bucketNumber: result.bucketNumber,
